...
exports.updateUserInfo = (req, res) => {
  // 定义待执行的 SQL 语句
  const sql = `update bilibili_user set ? where id=?`
...
exports.updatePassword = (req, res) => {
  // 根据 id 查询用户的信息
  const sql = `select * from bilibili_user where id=?`
  ...
  db.query(sql, req.user.id, (err, results) => {
    ...
    // 定义更新密码的 SQL 语句
    const sql = `update bilibili_user set password=? where id=?`
    // 对新密码进行加密处理
...
exports.updateAvatar = (req, res) => {
  const sql = `update bilibili_user set user_pic=? where id=?`
